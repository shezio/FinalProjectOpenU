...
import { exportPettyCashReportToExcel, exportPettyCashReportToPDF } from '../../components/export_utils';
...
const PettyCashReport = () => {
  ...
  const fetchEntries = () => {
    ...
    axios.get('/api/petty-cash/')
      .then(res => setEntries(res.data.petty_cash || []))
      .catch(() => showErrorToast('שגיאה בטעינת נתוני הקופה הקטנה'))
  ...
  return (
    <div className="refunds-main-content">
      <Sidebar />
      <InnerPageHeader title="קופה קטנה לפי תקופה" />

      {/* Controls: back + export + filters — all in one row */}
      <div className="refunds-controls back-controls">
        <button onClick={() => navigate('/petty-cash')}>← חזרה לקופה הקטנה</button>
        ...
        <button
          ...
          onClick={() => exportPettyCashReportToExcel(entries, period, selectedYear)}
        >
          <img src="/assets/excel-icon.png" alt="Excel" />
        </button>
        <button
          className="export-button pdf-button"
          onClick={() => exportPettyCashReportToPDF(entries, period, selectedYear)}
        >
                              ...
                              <tbody>
                                {paginated.map(r => (
                                  <tr key={r.id}>
                                    ...
                                    <td>{r.source_refund_id ? `מהחזר #${r.source_refund_id}` : '—'}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
